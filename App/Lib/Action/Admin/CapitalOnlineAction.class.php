...
class CapitalOnlineAction extends ACommonAction
{
    ...
    */
     ...
     public function withdraw()
    {
		...
		$list = M('member_withdraw w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->join("{$this->pre}member_money mm on w.uid = mm.uid")->where($map)->limit($Lsql)->order("w.id DESC")->select();
